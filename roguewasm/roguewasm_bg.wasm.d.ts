/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export function __wbg_playercore_free(a: number, b: number): void;
export function playercore_new(a: number, b: number, c: number, d: number, e: number, f: number, g: number): number;
export function playercore_x(a: number): number;
export function playercore_y(a: number): number;
export function playercore_draw(a: number): void;
export function playercore_emit_stats(a: number): void;
export function playercore_move_to(a: number, b: number, c: number): void;
export function playercore_take_damage(a: number, b: number): number;
export function __wbg_engine_free(a: number, b: number): void;
export function engine_new(a: number): number;
export function engine_open_box(a: number, b: number, c: number, d: number): void;
export function engine_on_dig(a: number, b: number, c: number, d: number): void;
export function engine_place_box(a: number, b: number, c: number): void;
export function engine_mark_box_as_prize(a: number, b: number, c: number): void;
export function engine_draw_map(a: number): void;
export function engine_is_cell_free(a: number, b: number, c: number): number;
export function engine_redraw_at(a: number, b: number, c: number): void;
export function engine_move_player(a: number, b: number, c: number, d: number): void;
export function __wbindgen_malloc(a: number, b: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number, d: number): number;
